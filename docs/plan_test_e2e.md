...
## 3) Observabilité des requêtes (recommandé)

Pour chaque test CRUD, ajouter des `cy.intercept()` :

- `POST /api/login` → `@login`
- `GET /api/student` → `@getAll`
- `POST /api/student` → `@create`
- `GET /api/student/*` → `@getById`
- `PUT /api/student/*` → `@update`
- `DELETE /api/student/*` → `@delete`

**Attendus techniques :**

- Statut HTTP OK (200/201/204)
- Payload minimal cohérent (quand applicable)
